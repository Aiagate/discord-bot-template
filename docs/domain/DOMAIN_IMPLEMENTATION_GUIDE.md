...
- ✅ **ビジネスルールの定義**: ドメインの不変条件（Invariants）を保証
- ✅ **エンティティと集約の管理**: ドメインオブジェクトのライフサイクル管理
- ✅ **フレームワーク非依存**: 純粋なPythonオブジェクトとして実装
- ❌ **データベースアクセスは行わない**: インフラストラクチャ層の責務
- ❌ **外部APIを呼び出さない**: インフラストラクチャ層の責務
- ❌ **UIロジックを持たない**: プレゼンテーション層の責務
...
```python
# app/domain/interfaces/auditable.py
from datetime import datetime
from typing import Protocol, runtime_checkable
...
### ❌ アンチパターン1: 貧血ドメインモデル（Anemic Domain Model）
...
- ✅ `@dataclass` を使用してシンプルに実装
- ✅ `__post_init__` でバリデーションを実装
- ✅ ビジネスロジックはドメインメソッドに実装
- ✅ フレームワーク非依存を保つ
- ✅ 型ヒントとDocstringを記述
- ✅ タイムスタンプが必要な場合は`IAuditable`を実装
- ✅ 不変条件を常に保証
- ✅ 集約境界を尊重
- ❌ データベースアクセスを行わない
- ❌ 外部APIを呼び出さない

...
class ApplicationMediator:
    """Expose the configured mediator to presentation adapters."""
    ...
    def send_async[T](
        self,
        request: Request[Result[T, UseCaseError]],
    ) -> AwaitableResult[T, UseCaseError]:
        """Dispatch a request through the application's mediator."""
        return self._mediator.send_async(request)
